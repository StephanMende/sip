...
class ShowHideButton extends React.Component {
    ...
    render() {
        ...
        return (
            <button onClick = {this.hideContent} style = {style.inactive}>
                Hide content
            < /button>
        );
    ...
    render() {
        ...
        return (
            ...
            {this.state.status === 1 &&
                    <div>
                    <ShowHideButton />
            <div style={style.content} dangerouslySetInnerHTML={{__html: this.props.content }}/></div>
            }
        {this.state.status === 0 &&
        <button onClick={this.showContent} style={style.active}>
                Show more
        </button>
        }
    </div>
